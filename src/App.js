                      ...
                      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
                        ...
                        <button onClick={async(e)=>{
                            ...
                            try {
                              ...
                              const html = "<!DOCTYPE html><html><head><meta charset=\"UTF-8\"/>"
                                ...
                                + "<table><thead><tr><th>S.No</th><th>LOT No</th><th>Grower</th><th>Father</th><th>Village</th><th>Variety</th><th>Packets</th><th>Result</th><th>Type</th><th>Rate</th><th>Note</th></tr></thead>"
                                + "<tbody>" + rowsHtml
                                + "<tr class=\"total-row\"><td colspan=\"6\">TOTAL</td><td style=\"text-align:center;padding:6px 8px;border:1px solid #ddd\">" + totalPkts.toLocaleString("en-IN") + "</td><td colspan=\"4\"></td></tr>"
                        ...
                        <button onClick={()=>{
                          ...
                          growers.forEach((g,i)=>{
                            ...
                            const ip=g.result==="Pass";
                            const amt=ip?(parseFloat(g.packets)||0)*vpR:0;
                            rowsHtml+="<tr style=\"background:"+(ip?i%2===0?"#fff":"#f5faff":"#fdecea")+"\">"
                              +"<td style=\"padding:5px 8px;border:1px solid #ddd;text-align:center\">"+(g.sNo||i+1)+"</td>"
                              +"<td style=\"padding:5px 8px;border:1px solid #ddd\">"+(g.lotNo||"")+"</td>"
                              +"<td style=\"padding:5px 8px;border:1px solid #ddd;font-weight:600\">"+(g.name||"")+"</td>"
                              +"<td style=\"padding:5px 8px;border:1px solid #ddd\">"+(g.fatherName||"")+"</td>"
                              +"<td style=\"padding:5px 8px;border:1px solid #ddd\">"+(g.village||"")+"</td>"
                              +"<td style=\"padding:5px 8px;border:1px solid #ddd\">"+(g.variety||"")+"</td>"
                              +"<td style=\"padding:5px 8px;border:1px solid #ddd;text-align:center\">"+(ip?parseFloat(g.packets)||0:"—")+"</td>"
                              +"<td style=\"padding:5px 8px;border:1px solid #ddd;text-align:center;font-weight:700;color:"+(ip?"#155724":"#721c24")+"\">"+(ip?"✓P":"✗F")+"</td>"
                              +"<td style=\"padding:5px 8px;border:1px solid #ddd;text-align:center\">"+vpT+"</td>"
      ...
      {mode==="careof"&&(()=>{
        ...
        const printCareOfSummary = (name, farmersList) => {
          ...
          const rows = farmersList.map(f => {
            ...
            const balColor = bal>=0?'#1a7a1a':'#b30000';
            const balText = '₹'+Math.abs(Math.round(bal)).toLocaleString('en-IN')+' '+(bal>=0?'(Pay)':'(Due)');
            return '<tr>'
              +'<td style="'+td+'">'+(f.farmerNo||'')+'</td>'
              +'<td style="'+td+'">'+(f.name||'')+'</td>'
              +'<td style="'+td+'">'+(f.village||'')+'</td>'
              +'<td style="'+td+'">'+varietyText+'</td>'
              +'<td style="'+td+'text-align:center;">'+(totalQty>0?totalQty.toLocaleString('en-IN'):'—')+'</td>'
              +'<td style="'+td+'text-align:right;font-weight:700;color:'+balColor+'">'+balText+'</td>'
              +'</tr>';
          }).join('');
