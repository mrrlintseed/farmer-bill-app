...
// Compound interest — compounds yearly with safety checks
function calcCompoundInterest(amount, rate, fromDate, billDate) {
  try {
    const amt = parseFloat(amount) || 0;
    const r = parseFloat(rate) || 0;
    if (!amt || !r || !fromDate) return { days: 0, interest: 0 };
    const fallbackDate = localStorage.getItem("app_bill_date") || "2026-07-01";
    const from = parseLocalDate(fromDate);
    const to = parseLocalDate(billDate || fallbackDate);
    const totalDays = Math.max(1, Math.round((to - from) / 86400000));
    if (totalDays < 365) {
      return { days: totalDays, interest: Math.round((amt * r * totalDays) / (100 * 30 * 12)) };
    }
                  ...
                  {subOrgTab==="growers"&&(
                    <div>
                      <div style={{ display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10 }}>
                        ...
                        <button onClick={()=>updateSO({...so,growers:[...growers,{sNo:growers.length+1,lotNo:"",name:"",fatherName:"",village:so.village||"",variety:"",area:"",packets:0,result:"Pass",type:"KMS",rate:0}]})} style={{background:"#2d5a8a",color:"#fff",border:"none",borderRadius:5,padding:"6px 14px",cursor:"pointer",fontSize:12}}>+ Add Grower</button>
                      </div>
                      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
                        ...
                        <button onClick={async(e)=>{
                            ...
                            try {
                              ...
                              const teluguGrowers = growers.map((g,i)=>({...g,name:tr[i*3]||g.name,fatherName:tr[i*3+1]||g.fatherName,village:tr[i*3+2]||g.village}));
                              const rows=teluguGrowers.map((g,idx)=>{const vpR=getSubOrgVarietyRate(growers[idx].variety)||parseFloat(growers[idx].rate)||0;const vpT=getSubOrgVarietyType(growers[idx].variety)||(growers[idx].type||"KMS");const ip=growers[idx].result==="Pass";const amt=ip?(parseFloat(growers[idx].packets)||0)*vpR:0;return `<tr style="background:${ip?idx%2===0?"#fff":"#f5faff":"#fdecea"}"><td style="padding:5px 8px;border:1px solid #ddd;text-align:center;">${growers[idx].sNo||idx+1}</td><td style="padding:5px 8px;border:1px solid #ddd;">${growers[idx].lotNo||""}</td><td style="padding:5px 8px;border:1px solid #ddd;font-weight:600;">${g.name||""}</td><td style="padding:5px 8px;border:1px solid #ddd;">${g.fatherName||""}</td><td style="padding:5px 8px;border:1px solid #ddd;">${g.village||""}</td><td style="padding:5px 8px;border:1px solid #ddd;">${growers[idx].variety||""}</td><td style="padding:5px 8px;border:1px solid #ddd;text-align:center;">${ip?parseFloat(growers[idx].packets)||0:"—"}</td><td style="padding:5px 8px;border:1px solid #ddd;text-align:center;font-weight:700;color:${ip?"#155724":"#721c24"}">${ip?"✓P":"✗F"}</td><td style="padding:5px 8px;border:1px solid #ddd;text-align:center;">${vpT}</td><td style="padding:5px 8px;border:1px solid #ddd;text-align:right;">₹${vpR.toLocaleString("en-IN")}</td><td style="padding:5px 8px;border:1px solid #ddd;text-align:right;font-weight:600;color:#1a6a1a;">${ip?"₹"+amt.toLocaleString("en-IN"):"—"}</td><td style="padding:5px 8px;border:1px solid #ddd;font-size:11px;color:#856404;">${growers[idx].note||""}</td></tr>`;}).join("");
                              ...
                              const html=`<!DOCTYPE html><html><head><meta charset="UTF-8"/><link href="https://fonts.googleapis.com/css2?family=Noto+Serif+Telugu&display=swap" rel="stylesheet"/><title>Growers (Telugu) — ${so.name}</title><style>body{font-family:"Noto Serif Telugu",Georgia,serif;padding:20px;}h2{color:#1a2a4a;}table{border-collapse:collapse;width:100%;font-size:12px;}th{background:#1a2a4a;color:#fff;padding:6px 8px;border:1px solid #ddd;}.total-row td{font-weight:800;background:#e8f0ff;border-top:2px solid #1a2a4a;}@media print{@page{margin:8mm;size:A4 landscape;}}</style></head><body><h2>Growers — ${so.name} (#${so.accNo})</h2><table><thead><tr><th>S.No</th><th>LOT No</th><th>Grower</th><th>Father</th><th>Village</th><th>Variety</th><th>Packets</th><th>Result</th><th>Type</th><th>Rate</th><th>Amount</th><th>Note</th></tr></thead><tbody>${rows}<tr class="total-row"><td colspan="6">TOTAL</td><td style="text-align:center;padding:6px 8px;border:1px solid #ddd;">${totalPkts.toLocaleString("en-IN")}</td><td colspan="3"></td><td style="text-align:right;padding:6px 8px;border:1px solid #ddd;">₹${Math.round(totalAmt).toLocaleString("en-IN")}</td><td></td></tr></tbody></table></body></html>`;
                        ...
                        <button onClick={()=>{const rows=growers.map((g,i)=>{const vpR=getSubOrgVarietyRate(g.variety)||parseFloat(g.rate)||0;const vpT=getSubOrgVarietyType(g.variety)||(g.type||"KMS");const ip=g.result==="Pass";const amt=ip?(parseFloat(g.packets)||0)*vpR:0;return `<tr style="background:${ip?i%2===0?"#fff":"#f5faff":"#fdecea"}"><td style="padding:5px 8px;border:1px solid #ddd;text-align:center;">${g.sNo||i+1}</td><td style="padding:5px 8px;border:1px solid #ddd;">${g.lotNo||""}</td><td style="padding:5px 8px;border:1px solid #ddd;font-weight:600;">${g.name||""}</td><td style="padding:5px 8px;border:1px solid #ddd;">${g.fatherName||""}</td><td style="padding:5px 8px;border:1px solid #ddd;">${g.village||""}</td><td style="padding:5px 8px;border:1px solid #ddd;">${g.variety||""}</td><td style="padding:5px 8px;border:1px solid #ddd;text-align:center;">${ip?parseFloat(g.packets)||0:"—"}</td><td style="padding:5px 8px;border:1px solid #ddd;text-align:center;font-weight:700;color:${ip?"#155724":"#721c24"}">${ip?"✓P":"✗F"}</td><td style="padding:5px 8px;border:1px solid #ddd;text-align:center;">${vpT}</td><td style="padding:5px 8px;border:1px solid #ddd;text-align:right;">₹${vpR.toLocaleString("en-IN")}</td><td style="padding:5px 8px;border:1px solid #ddd;text-align:right;font-weight:600;color:#1a6a1a;">${ip?"₹"+amt.toLocaleString("en-IN"):"—"}</td><td style="padding:5px 8px;border:1px solid #ddd;font-size:11px;color:#856404;">${g.note||""}</td></tr>`;}).join("");const totalPkts=growers.filter(g=>g.result==="Pass").reduce((s,g)=>s+(parseFloat(g.packets)||0),0);const totalAmt=growers.filter(g=>g.result==="Pass").reduce((s,g)=>s+(parseFloat(g.packets)||0)*(getSubOrgVarietyRate(g.variety)||parseFloat(g.rate)||0),0);const html=`<!DOCTYPE html><html><head><meta charset="UTF-8"/><title>Growers — ${so.name}</title><style>body{font-family:Georgia,serif;padding:20px;}h2{color:#1a2a4a;}table{border-collapse:collapse;width:100%;font-size:12px;}th{background:#1a2a4a;color:#fff;padding:6px 8px;border:1px solid #ddd;}.total-row td{font-weight:800;background:#e8f0ff;border-top:2px solid #1a2a4a;}@media print{@page{margin:8mm;size:A4 landscape;}}</style></head><body><h2>Growers — ${so.name} (#${so.accNo})</h2><div style="font-size:13px;color:#555;margin-bottom:8px;">Village: ${so.village||"—"} | Total: ${growers.length} growers</div><table><thead><tr><th>S.No</th><th>LOT No</th><th>Grower</th><th>Father</th><th>Village</th><th>Variety</th><th>Packets</th><th>Result</th><th>Type</th><th>Rate</th><th>Amount</th><th>Note</th></tr></thead><tbody>${rows}<tr class="total-row"><td colspan="6">TOTAL</td><td style="padding:6px 8px;border:1px solid #ddd;text-align:center;">${totalPkts.toLocaleString("en-IN")}</td><td colspan="3"></td><td style="padding:6px 8px;border:1px solid #ddd;text-align:right;">₹${Math.round(totalAmt).toLocaleString("en-IN")}</td><td></td></tr></tbody></table></body></html>`;const w=window.open("","_blank");w.document.write(html);w.document.close();setTimeout(()=>w.print(),400);}} style={{background:"#1a2a4a",color:"#fff",border:"none",borderRadius:5,padding:"5px 14px",fontSize:12,fontWeight:700,cursor:"pointer"}}>🖨️ Print Growers</button>
                        </div>
                        </div>
                      </div>
